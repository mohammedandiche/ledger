...
export type AccountActionSheetProps = {
...
export function AccountActionSheet({ account, onDismiss, onRename, onToggleClose, onDelete }: AccountActionSheetProps) {
  const { r } = useR();
  ...
  const isClosed = account.closed ?? false;

  return (
    <BottomSheetModal visible={!!account} onDismiss={onDismiss} statusBarTranslucent>
      ...
      >
        <View style={[s.icon, { backgroundColor: colors.s2 }]}>
          <Ionicons
      ...
      >
        ...
        <Text style={[s.rowText, s.rowRed, { fontSize: r(13, 15) }]}>Delete account</Text>
      </Pressable>
